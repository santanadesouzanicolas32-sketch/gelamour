...
async function finalizarPedido() {
    ...
    document.querySelectorAll('.btn-pedir').forEach(btn => {
        ...
        if (!m)
            return;
        if (carrinho[m[1]])
            carrinho[m[1]].preco = parseFloat(m[2]);
    });
...
Pedido pelo cardápio online ✨`;
    ...
    try {
        ...
        if (_idMatch) {
            ...
            if (btnFin)
                btnFin.textContent = '✅ Pedido registrado!';
            salvarEnderecoCliente(endereco);
        }
    }
    catch (_e) {
        if (btnFin)
            btnFin.textContent = '⚠️ Erro - pedido só no WhatsApp';
...
async function iniciarFluxoPix(pedidoId, total, nome, msgWA, billingType, itens, endereco) {
    ...
    fecharModal();
    if (!isPix)
        return; // Cartão: usuário preenche o form e clica "Pagar agora"
...
async function pagarCartao() {
    ...
    if (cpf.length !== 11) {
        alert('CPF inválido (somente números, 11 dígitos).');
        return;
    }
    if (!validade.includes('/') || validade.length !== 5) {
...
function mostrarReciboPix() {
    ...
    const linhasItens = _pixItens.map(i => '<div class="recibo-item"><span>' + escHTML(i.nome) + '</span><span>R$ ' + Number(i.preco).toFixed(2).replace('.', ',') + '</span></div>').join('');
...
function carouselNext(id, e) {
    ...
    let cur = 0;
    imgs.forEach(function (img, i) { if (img.classList.contains('ativo'))
        cur = i; });
...
function dbFetch(url, opts) {
    ...
    return fetch(url, Object.assign(Object.assign({}, opts), { signal: ctrl.signal }))
        .then(r => { clearTimeout(tid); return r; })
        .catch(e => { clearTimeout(tid); if (e.name === 'AbortError')
        throw new Error('Tempo limite excedido'); throw e; });
}
...
async function verificarTelefone() {
    ...
    const erro = document.getElementById('loginErro');
    if (tel.length < 10) {
        erro.textContent = 'Digite um número válido com DDD.';
        erro.style.display = 'block';
        return;
    }
    ...
    try {
        ...
        if (_loginTentativas >= 5) {
            _loginBloqueioAte = Date.now() + 60000;
            _loginTentativas = 0;
        }
...
async function cadastrar() {
    ...
    try {
        ...
        btn.textContent = 'Entrar no cardápio ✨';
        btn.disabled = false;
        _cadastrando = false;
    }
}
...
function entrarComCliente(cliente) {
    ...
    if (cliente.endereco)
        document.getElementById('inpEndereco').value = cliente.endereco;
}
function sair() {
    ...
    try {
        ...
        if (data && data[0]) {
            ...
            if (cfg.premios && Array.isArray(cfg.premios) && cfg.premios.length > 0) {
                ...
                if (grid) {
                    const icones = ['🍫', '🧁', '🚚', '💸', '💰', '🎉', '🍮', '🎀', '🌟'];
                    grid.innerHTML = _roletaPremios.map((p, i) => `<div class="roleta-premio-item">${icones[i % icones.length]} ${escHTML(p)}</div>`).join('');
                }
            }
        }
    }
...
async function verificarStatusRoleta() {
    if (!clienteAtual)
        return null;
    try {
        const r = await dbFetch(SUPABASE_URL + '/rest/v1/roleta_participacoes?cliente_id=eq.' + clienteAtual.id + '&order=created_at.desc&limit=1', { headers: { apikey: SUPABASE_ANON, Authorization: 'Bearer ' + SUPABASE_ANON } });
        if (!r.ok)
            return null;
        const data = await r.json();
        if (!data || !data[0])
...
function atualizarUIRoleta(info) {
    ...
    if (info.status === 'pendente') {
        ...
        if (girarBtn) {
            girarBtn.disabled = true;
            girarBtn.style.opacity = '0.4';
            girarBtn.title = 'Aguardando aprovação';
        }
    }
        ...
        if (!aprovadoHoje) {
            ...
            if (girarBtn) {
                girarBtn.disabled = true;
                girarBtn.style.opacity = '0.4';
                girarBtn.textContent = '🔒 Prazo expirado';
            }
        }
        else {
...
async function registrarParticipacao(instagram) {
    ...
    try {
        ...
        if (check && check.status !== 'rejeitado')
            return;
...
async function salvarVencedor(premio) {
    if (!clienteAtual || !_roletaParticipacaoId)
        return;
    if (isContaTeste())
        return; // conta teste: não registra no banco
    try {
...
function fecharRoletaAdminBackdrop(e) {
    if (e.target.id === 'roletaAdminBackdrop')
        fecharRoletaAdmin();
}
function abrirTabAdmin(tab, btn) {
    ...
    if (tab === 'pendentes')
        carregarParticipantesRoleta();
    else if (tab === 'aprovados')
        carregarAprovadosRoleta();
    else if (tab === 'vencedores')
        carregarVencedoresRoleta();
    else if (tab === 'config')
        carregarConfigAdmin();
}
async function carregarParticipantesRoleta() {
    ...
    try {
        const r = await dbFetch(SUPABASE_URL + '/rest/v1/roleta_participacoes?status=eq.pendente&order=created_at.desc', { headers: { apikey: SUPABASE_ANON, Authorization: 'Bearer ' + SUPABASE_ANON } });
        const data = await r.json();
        if (!data || !data.length) {
...
async function carregarAprovadosRoleta() {
    ...
    el.innerHTML = '<div class="roleta-empty">Carregando...</div>';
    try {
        const r = await dbFetch(SUPABASE_URL + '/rest/v1/roleta_participacoes?status=eq.aprovado&order=data_aprovacao.desc', { headers: { apikey: SUPABASE_ANON, Authorization: 'Bearer ' + SUPABASE_ANON } });
        const data = await r.json();
        if (!data || !data.length) {
...
async function rejeitarParticipante(id, btn) {
    if (!confirm('Rejeitar esta participação?'))
        return;
    btn.disabled = true;
    btn.textContent = '...';
    try {
...
async function carregarVencedoresRoleta() {
    ...
    el.innerHTML = '<div class="roleta-empty">Carregando...</div>';
    try {
        const r = await dbFetch(SUPABASE_URL + '/rest/v1/roleta_vencedores?order=data_vitoria.desc', { headers: { apikey: SUPABASE_ANON, Authorization: 'Bearer ' + SUPABASE_ANON } });
        const data = await r.json();
        if (!data || !data.length) {
